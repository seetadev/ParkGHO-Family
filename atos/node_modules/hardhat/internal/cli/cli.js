...
async function suggestInstallingHardhatVscode() {
    ...
    if (installationConsent === true) {
        ...
        if (installed) {
            ...
            console.log("Hardhat for Visual Studio Code couldn't be installed. To learn more about it, go to https://v2.hardhat.org/hardhat-vscode");
        }
    }
    else {
        console.log("To learn more about Hardhat for Visual Studio Code, go to https://v2.hardhat.org/hardhat-vscode");
    }
}
...
async function main() {
    ...
    try {
        ...
        if (showStackTraces || SHOULD_SHOW_STACK_TRACES_BY_DEFAULT) {
            ...
            if (!isHardhatError) {
                console.error(`If you think this is a bug in Hardhat, please report it here: https://v2.hardhat.org/report-bug`);
            }
            if (errors_1.HardhatError.isHardhatError(error)) {
                const link = `https://v2.hardhat.org/${(0, errors_list_1.getErrorCode)(error.errorDescriptor)}`;
