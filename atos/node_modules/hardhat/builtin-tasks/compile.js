    ...
    try {
        ...
        try {
            if (fs_extra_1.default.lstatSync(absolutePath).isDirectory()) {
                err = new errors_1.HardhatError(errors_list_1.ERRORS.GENERAL.INVALID_READ_OF_DIRECTORY, {
                    absolutePath,
                });
            }
        }
...
(0, config_env_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_LOG_COMPILATION_ERRORS)
    ...
    if (hasConsoleErrors) {
        console.error(picocolors_1.default.red(`The console.log call you made isn’t supported. See https://v2.hardhat.org/console-log for the list of supported methods.`));
        console.log();
    }
});
/**
...
(0, config_env_1.subtask)(task_names_1.TASK_COMPILE_SOLIDITY_GET_COMPILATION_JOBS_FAILURE_REASONS)
    ...
    const incompatibleOverriddenSolc = [];
    ...
    for (const error of errors) {
        if (error.reason ===
            ...
            builtin_tasks_1.CompilationJobCreationErrorReason.INCOMPATIBLE_OVERRIDDEN_SOLC_VERSION) {
