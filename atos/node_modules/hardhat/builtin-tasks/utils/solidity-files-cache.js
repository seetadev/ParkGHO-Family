    ...
    static async readFromFile(solidityFilesCachePath) {
        ...
        if (await fs_extra_1.default.pathExists(solidityFilesCachePath)) {
            try {
                ...
                log("Failed to read solidity files cache at %s. Removing it. %O", solidityFilesCachePath, error);
                await fs_extra_1.default
                    .remove(solidityFilesCachePath)
                    .catch((removalError) => log("Failed to remove invalid solidity files cache at %s: %O", solidityFilesCachePath, removalError));
            }
        }
