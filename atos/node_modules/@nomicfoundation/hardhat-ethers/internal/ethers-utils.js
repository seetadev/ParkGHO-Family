...
function authorizationListify(authorizationList) {
    return authorizationList.map((el) => {
        const auth = (0, ethers_1.authorizationify)(el);
        return {
            address: auth.address,
            nonce: (0, ethers_1.toQuantity)(auth.nonce),
            chainId: (0, ethers_1.toQuantity)(auth.chainId),
            r: auth.signature.r,
            s: auth.signature.s,
            yParity: (0, ethers_1.toQuantity)(auth.signature.yParity),
        };
    });
}
function convertToTxAuthorizationList(authorizationList) {
    const authorizations = [];
    for (const auth of authorizationList) {
        authorizations.push({
            address: auth.address,
            nonce: (0, ethers_1.getBigInt)(auth.nonce),
            chainId: (0, ethers_1.getBigInt)(auth.chainId),
            signature: ethers_1.Signature.from(auth),
        });
    }
